...
impl View for PpuView {
    fn init(&mut self, _ctx: &Context, control: &Sender<nes::ControlMessage>) {
    ...
    fn window(&mut self, ctx: &Context) {
        ...
        egui::Window::new("PPU")
            ...
            .open(opened)
            .show(ctx, |ui| {
                if *state == EmulationState::Run(None) {
                ...
                egui::Grid::new("ppugeneral").show(ui, |ui| {
                    ui.label("Cycle");
                    ui.label(format!("{}", ppu_state.cycle));
                    ui.end_row();
                    ui.label("Frame");
                    ui.label(format!("{}", ppu_state.frame));
                    ui.end_row();
                    ui.label("Scanline");
                    ui.label(format!("{}", ppu_state.scanline));
                    ui.end_row();
                    ui.label("Pixel");
                    ui.label(format!("{}", ppu_state.pixel));
                    ui.end_row();
                    ...
                    ui.label("PPUCTRL");
                    ui.label(format!("0x{:02x}", *ppu_state.ctrl));
                    ui.end_row();
                    ui.label("PPUMASK");
                    ui.label(format!("0x{:02x}", *ppu_state.mask));
                    ui.end_row();
                    ui.label("PPUSTATUS");
                    ui.label(format!("0x{:02x}", ppu_state.status.get()));
                    ui.end_row();
                    ui.label("OAMADDR");
                    ui.label(format!("0x{:02x}", ppu_state.oamaddr));
                    ui.end_row();
                    ui.label("OAMDATA");
                    ui.label(format!("0x{:02x}", ppu_state.oamdata));
                    ui.end_row();
                    ui.label("PPUADDR");
                    ui.label(format!("0x{:02x}", ppu_state.addr));
                    ui.end_row();
                    ui.label("PPUDATA");
                    ui.label(format!("0x{:02x}", ppu_state.data));
                    ui.end_row();
                    ui.label("OAMDMA");
                    ui.label(format!("0x{:02x}", ppu_state.oamdma));
                    ui.end_row();
